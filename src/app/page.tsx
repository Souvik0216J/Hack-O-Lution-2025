...
export default function Home() {
  // Time counter
  const eventDate = new Date('June 27, 2025 14:30:00')
  ...
  return (
    <>
      ...
      {/* Judges Section */}
      <section id="judges" className="pt-20 pb-16">
        <div className="container mx-auto px-4">
          <div className="flex flex-col justify-center items-center text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 glitch-text-sm text-center" data-text="HACKOLUTION MENTORS">
              HACK&#123;<span className="text-green-400">0</span>&#125;LUTION JUDGES
            </h2>
            <p className="text-muted-foreground max-w-3xl mx-auto">
                    ...
                    {judges.image ? (
                      <Image
                        ...
                        onError={() => {
                          ...
                          console.log(`Failed to load image for ${judges.name}`);
                        }}
                      />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-2xl font-bold text-white">
                        {judges.name.split(' ').map(n => n[0]).join('')}
                      </div>
                    )}
                  </div>
                  <h3 className="text-xl font-bold text-white mb-2">{judges.name}</h3>
                  <p className="text-green-400 text-sm mb-4">{judges.title}</p>

                  {/* LinkedIn Link */}
                  {judges.linkedin && (
                    <div className="flex justify-center">
                      <a
                        href={judges.linkedin}
                        target="_blank"
